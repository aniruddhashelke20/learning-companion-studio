...
router.post('/register', async (req, res, next) => { try {
  ...
  req.user = user; await logEvent(req, { eventName: 'USER_REGISTERED', component: 'System', description: `User ${user.email} registered` });
...
router.post('/login', async (req, res, next) => { try {
  ...
  req.user = user; await logEvent(req, { eventName: 'USER_LOGGED_IN', component: 'System', description: `User ${user.email} logged in` });
